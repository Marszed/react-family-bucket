...
class RadioAuthorize extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.clearData && nextProps.clearData.clear && !isEqual(nextProps.clearData, this.state.clearData)) {
            this.setState({
                data: Object.assign(objCopy(this.state.data),{defaultValue: false}),
    ...
    render() {
        return (
            <label className={"ipx_checkbox float_rt" + (this.state.data.defaultValue ? " checked" : "")} onClick={this.onChange.bind(this)}>
                <i className={"iconfont" + (this.state.data.defaultValue ? " icon-succeed" : "")}/><span className="text-elps" style={{'paddingLeft':'5px'}}>{this.state.data.description}</span>
            </label>
        );
    }
}
