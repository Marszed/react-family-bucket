                                                ...
                                                ((Number(obj.isAbroad) === Number(this.state.isAbroad)) || this.state.isAbroad === -1))) ? <li key={obj.propertyId} className={statusClass[Number(obj.propertyStatus) - 1]} onClick={this.viewPropertyDetail.bind(this, obj)}>
                                                        <h2>#{obj.lot}</h2>
                                                        {
                                                            projectType != 4 ? <ol className={this.state.fontSize <= 36 ? 'hide' : ''}>
                                                                ...
                                                                {/*建筑面积(公寓，独栋别墅，联排别墅用)*/}
                                                                {
                                                                    (projectType != 4) ? <li title={messages.constructionArea + ': ' + (obj.constructionArea)}>{obj.constructionArea} {obj.areaUnit}</li> : null
                                                                }
                                                                {/*土地面积(土地，独栋别墅用)*/}
                                                                {
                                                                    (projectType == 2 || projectType == 4) ? <li title={messages.landArea + ': ' + (obj.landArea)}>{obj.landArea} {obj.areaUnit}</li> : null
                                                                }
